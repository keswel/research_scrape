...
while (-not $PHPSESSID) {
    ...
    if (-not $LT -or -not $EXEC) {
        Write-Warning "Failed to grab login tokens. Try again."
        continue
    }
    ...
    if (-not $ticketUrl) {
        Write-Warning "Login failed - check your username and password, then try again."
        continue
    }
    ...
    if (-not $PHPSESSID) {
        Write-Warning "Failed to get PHPSESSID. Try again."
    }
}
